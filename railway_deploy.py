...
# ========== REGION_KEYWORDS (весь словарь) ==========
REGION_KEYWORDS = {
    'toshkent': { 'topic_id': 101362, 'keywords': [
        'toshkent','tashkent','ташкент','тошкент','bekobod','бекабад','бекобод','olmaliq','алмалык','олмалиқ','ahangaron','ахангаран','оҳангарон','angren','ангрен','chirchiq','чирчик','чирчиқ','yangiyul','яккасарай','chilonzor','чилонзор'
    ]},
    'fargona':  { 'topic_id': 101382, 'keywords': [
        "farg'ona",'fargona','фергана','qoqon','kokand','коканд','margilon','маргилан','quvasoy','кувасай','beshariq','бешарык'
    ]},
    'andijon':  { 'topic_id': 101387, 'keywords': [
        'andijon','andijan','андижан','asaka','асака','boz','боз','marhamat','мархамат'
    ]},
    'buxoro':   { 'topic_id': 101372, 'keywords': [
        'buxoro','bukhara','бухара','alat','алат',"g'ijduvon",'гиждуван','kogon','когон'
    ]},
    'namangan': { 'topic_id': 101383, 'keywords': [
        'namangan','наманган','pop','поп','chortoq','чартак','yangiqorgon','янгикурган'
    ]},
    'samarqand':{ 'topic_id': 101369, 'keywords': [
        'samarqand','samarkand','самарканд','urgut','ургут','kattaqorgon','каттакурган'
    ]},
    'qashqadaryo':{ 'topic_id': 101380, 'keywords': [
        'qarshi','карши','shahrisabz','шахрисабз','koson','косон','guzar','гузар'
    ]},
    'navoiy':   { 'topic_id': 101379, 'keywords': [
        'navoiy','navoi','навои','zarafshon','зарафшан','karmana','кармана'
    ]},
    'sirdaryo': { 'topic_id': 101378, 'keywords': [
        'guliston','гулистан','shirin','ширин','boyovut','баяут'
    ]},
    'jizzax':   { 'topic_id': 101377, 'keywords': [
        'jizzax','джизак','gallaaral','галляарал','pakhtakor','пахтакор','zomin','зомин'
    ]},
    'nukus':    { 'topic_id': 101376, 'keywords': ['nukus','нукус'] },
    'urganch':  { 'topic_id': 101375, 'keywords': ['urgench','urganch','ургенч'] },
    'xorazm':   { 'topic_id': 101660, 'keywords': [
        'xorazm','xorezm','хорезм','xiva','khiva','хива','shovot','шават'
    ]},
    'qoraqalpoq':{ 'topic_id': 101381, 'keywords': [
        'qoraqalpoq','каракалпакстан','turtkul','турткуль','khojeli','ходжейли'
    ]},
    'xalqaro':  { 'topic_id': 101367, 'keywords': [
        'russia','россия','moskva','москва','spb','питер','kazakhstan','казахстан','turkey','turkiye','стамбул','china','китай','dubai','дубай'
    ]},
    'surxondaryo':{ 'topic_id': 101363, 'keywords': [
...
def handle_admin_command(msg):
    ...
    if text in ('/start', 'старт'):
        ...
        send_message(ADMIN_USER_ID, f"🤖 Активен. Сообщений {message_count}. Uptime {h}ч {m}м")
...
def process_message(message):
    ...
    try:
        ...
        topic = 'xalqaro' if 'xalqaro' in {from_reg, to_reg} else (from_reg or to_reg)

        if not topic:
            # админу – кнопки выбора топика
...
Другие грузы: @logistika_marka"""
        ...
        topic_id = REGION_KEYWORDS[topic]['topic_id']
